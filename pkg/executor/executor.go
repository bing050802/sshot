...
func NewExecutor(host types.Host, groupName string) (*Executor, error) {
	...
	// 补充内置连接变量（仅在用户未定义时）
	if _, ok := vars["inventory_hostname"]; !ok {
		vars["inventory_hostname"] = host.Name
	}
	if _, ok := vars["inventory_host"]; !ok {
		if host.Address != "" {
			vars["inventory_host"] = host.Address
		} else if host.Hostname != "" {
			vars["inventory_host"] = host.Hostname
		}
	}
	if _, ok := vars["inventory_port"]; !ok {
		port := host.Port
		if port == 0 {
			port = 22
		}
		vars["inventory_port"] = port
	}
	if _, ok := vars["inventory_user"]; !ok {
		vars["inventory_user"] = host.User
	}
	if _, ok := vars["inventory_hostname_short"]; !ok {
		vars["inventory_hostname_short"] = strings.SplitN(host.Name, ".", 2)[0]
	}
